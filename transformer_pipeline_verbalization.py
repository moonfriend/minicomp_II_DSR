...
sees all signals in one unified string. The high-signal categoricals
(room_type, neighbourhood) lead the string and are stated explicitly; the weak
numeric columns are rendered as purely qualitative phrases (no raw digits), with
the bucket chosen relative to the training-set median.
"""
...
#
# Design notes (grounded in train_01.csv, not guessed):
#   - room_type and neighbourhood carry ~10x the signal of any numeric column
#     (mutual info 0.285 / 0.275 vs <0.08), so they lead the string and are
#     stated explicitly.
#   - The numeric columns are weak predictors, so they are rendered as purely
#     qualitative phrases with no raw digits. We deliberately do NOT encode a
#     price direction into them: e.g. availability runs POSITIVE with price in
#     this data, the opposite of the usual intuition, so we keep the phrasing
#     neutral (relative to the training median) and let the model learn it.
#   - minimum_nights has ~0 signal; kept minimal and easy to drop.

# Training-set medians, used only as a neutral "typical" anchor for the
# qualitative buckets (not the target label, so this is not leakage).
_MED = {
    "number_of_reviews": 5,
...
# High-signal categoricals: expanded into meaningful phrases.
_ROOM_TYPE_TEXT = {
    "entire home/apt": "an entire home or apartment, the whole private unit to themselves",
    "private room":    "a private room within a shared home",
    "shared room":     "a shared room, the most budget level of space",
}


def _room_type_text(val) -> str:
    rt = str(val or "").strip().lower()
    return _ROOM_TYPE_TEXT.get(rt, rt)


# Weak numerics: purely qualitative, no digits.
...
def _reviews_text(val) -> str:
    ...
    if n is None:
        return ""
    if n == 0:
        return "a brand-new listing with no reviews yet"
    med = _MED["number_of_reviews"]
    if n >= 4 * med:
        return "a very well-reviewed listing"
    if n >= med:
        return "a moderately reviewed listing"
    return "a lightly reviewed listing"
...
def _host_listings_text(val) -> str:
    ...
    if n is None:
        return ""
    if n == 1:
        return "run by an individual host who lists only this one place"
    if n <= 3:
        return "run by a small host with a handful of listings"
    if n <= 15:
        return "run by a professional host managing many listings"
    return "run by a large property-management operation"
...
def _availability_text(val) -> str:
    ...
    if n is None:
        return ""
    med = _MED["availability_365"]
    if n == 0:
        return "with a fully blocked calendar this year"
    if n <= med:
        return "with a tighter-than-typical calendar"
    if n <= 2 * med:
        return "with a moderately open calendar"
    return "with a much more open calendar than usual"


def _minimum_nights_text(val) -> str:
    # Near-zero signal — kept minimal. Delete this branch (and its row below)
    # if you want to drop the feature from the text entirely.
    n = _as_int(val)
    if n is None:
        return ""
    if n <= 1:
        return "bookable for a single night"
    if n <= 30:
        return "open to short stays"
    return "geared to long-term stays"
...
def row_to_text(row: dict) -> str:
    ...
    # Strongest signal first: room type, stated explicitly.
    rt = _room_type_text(row.get("room_type"))
    if rt:
        parts.append(f"This listing offers {rt}")

    # Second strongest: location, at full granularity.
    nb  = str(row.get("neighbourhood", "") or "").strip()
    nbg = str(row.get("neighbourhood_group", "") or "").strip()
    if nb and nbg:
        parts.append(f"located in the {nb} neighbourhood of {nbg}")
    elif nbg:
        parts.append(f"located in {nbg}")

    # Weak numerics: qualitative phrases only.
    for fn, key in [
